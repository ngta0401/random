...
=== Klassen und Enumerationen
Dieser Abschnitt stellt eine Vereinigung von Glossar und der Beschreibung von Klassen/Enumerationen dar. Jede Klasse und Enumeration wird in Form eines Glossars textuell beschrieben. Zusätzlich werden eventuelle Konsistenz- und Formatierungsregeln aufgeführt.
...
|**User**
|Ist die Darstellung einer realen Person (Basisklasse).

|**Registered User**
|Ist die Repräsentation einer realen Person, welche fest im System gespeichert ist und sich authentifizieren kann.

|**Member**
|Ist ein als "Mitglied" registrierter User mit Kundenkonto und Trainingsplan.

|**Chef**
|Ist ein als "Chef" registrierter User mit erweiterten Rechten (Admin).

|**Employee**
|Ist ein als "Mitarbeiter" registrierter User (Trainer, Thekenkraft, Putzkraft).

|**ItemCatalog**
|Ist eine Gruppierung von verkäuflichen Artikeln (`Items`).

|**Item**
|Repräsentiert einen einzelnen Gegenstand (Produkt) im Online-Katalog.

|**Food**
|Kategorie für essbare Produkte im Online-Shop (z. B. Riegel, Shakes).

|**Others**
|Kategorie für Artikel im Online-Shop, die kein Essen sind (z. B. Handtücher, Zubehör).

|**Inventory**
|Stellt das Lager dar und verwaltet den physischen Bestand der Items aus dem `ItemCatalog`.

|**Inventory-Item**
|Stellt die konkrete Anzahl (Menge) eines verfügbaren Items im `Inventory` dar.

|**Vacation**
|Der Urlaub, den Mitarbeiter beantragen können (Zeitraum und Status).

|**Exercise**
|Steht für einzelne Übungen (z. B. Bankdrücken), die man im Training machen kann.

|**Training Day**
|Repräsentiert die Wochentage, an denen das Studio geöffnet ist und Training stattfindet.

|**Trainingsplan**
|Ordnet einem Mitglied Übungen zu (Wann ist welches Training?).

|**Trainingsplanitem**
|Ein Eintrag im Trainingsplan: Verknüpft eine Übung (`Exercise`) mit Gewicht, Wiederholungen und Tag.

|**TrialTraining**
|Ein Probetraining für Gäste, um das Fitnessstudio an einem Tag zu testen.

|**Shift**
|Eine Dienstschicht im Dienstplan, der Mitarbeiter und eine Zeitspanne zugeordnet sind.

|**AboPlan**
|(Enumeration) Definiert die Länge und Art eines Abos (z. B. `MONTHLY`, `YEARLY`).

|**CreatorCode**
|Möglichkeit für den Chef, wichtigen Mitgliedern eine Beteiligung an Neuanmeldungen/Bestellungen zu geben (Rabatt-Code).
|===
